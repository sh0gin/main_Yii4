...
$(() => {
  ...
  banTimeButton(); // заблокировать на время 0
  deleteComment(); // удалить комментарий 0
  permanent(); // постоянная блокировка 0 
  answerButton(); // кнопка ответить на комментарии 0 4556
  moreButton(); // кнопка Подробнее... 0
  edit(); // окно показа редактирования поста 0
  answerShow(); // кнопка ответить на комментарий 0 
  giveNumPagination(); // срабатывает когда жмём на кнопки пагинации 0
  giveInputComment(); // селектор на нажатии кнопки создания комментария 0
  getUserStatus(); // проверяет статус пользователя 0
});
